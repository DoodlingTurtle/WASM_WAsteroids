...
#include "../../assets_list.hpp"
    ...
    bool Assets::init(std::string* nextFile) {
        ...
        case Assets::SPRITESHEET:
            *loaderList[loadedAssetIndex].i.sh = new RGNDS::SpriteSheet();
            if (olc::OK != ((*loaderList[loadedAssetIndex].i.sh)->Load(loaderList[loadedAssetIndex].file))) {
                loadFailed = true;
                return false;
            } break;
    ...
    void Assets::deinit() {
        ...
        for (int a = 0; a < loadedAssetIndex; a++) {
            ...
            case Assets::SPRITESHEET:
                delete* loaderList[a].i.sh; break;
